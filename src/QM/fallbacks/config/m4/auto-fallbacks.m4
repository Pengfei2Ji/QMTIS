# Generated by make-macros-fallbacks on 2016/05/22 07:10:21 +0000
...
AC_DEFUN([ABI_FALLBACK_ATOMPAW],[
  ...
  atompaw_pkg_name="atompaw-4.0.0.13"
  AC_SUBST(atompaw_pkg_name)
  atompaw_pkg_string="AtomPAW version 4.0.0.13 (upstream release)"
  ...
  AC_ARG_ENABLE([ATOMPAW],
    AC_HELP_STRING([--enable-atompaw],
      [AtomPAW version 4.0.0.13 (upstream release)]))
  ...
  if test "${enable_atompaw}" = "yes"; then
    ...
    if test "${with_atompaw_libs}" = ""; then
      ...
      if test -s "${abinit_tardir}/atompaw-4.0.0.13.tar.gz"; then
        AC_MSG_RESULT([yes])
        AX_CHECK_MD5SUM([${abinit_tardir}/atompaw-4.0.0.13.tar.gz],[af4a042380356f6780183c4b325aad1d])
        case "${abi_md5_ok}" in
          yes)
            afb_tarball="yes"
      ...
      if test "${afb_ready}" = "no"; then
        for dl_url in "http://www.wfu.edu/~natalie/papers/pwpaw/atompaw-4.0.0.13.tar.gz" "http://forge.abinit.org/fallbacks/atompaw-4.0.0.13.tar.gz"; do
          if test "${afb_tarball}" = "no"; then
            AC_MSG_NOTICE([downloading ATOMPAW - this may take a while])
            rm -f "${abinit_tardir}/atompaw-4.0.0.13.tar.gz"
            ${afb_downloader} ${afb_dlopts}               "${abinit_tardir}/atompaw-4.0.0.13.tar.gz"               "${dl_url}"
            if test -s "${abinit_tardir}/atompaw-4.0.0.13.tar.gz"; then
              AX_CHECK_MD5SUM([${abinit_tardir}/atompaw-4.0.0.13.tar.gz],[af4a042380356f6780183c4b325aad1d])
              case "${abi_md5_ok}" in
                yes)
...
AC_DEFUN([ABI_FALLBACK_BIGDFT],[
  ...
  dnl Define variables needed to build the package
  bigdft_pkg_name="bigdft-1.7.1.23"
  AC_SUBST(bigdft_pkg_name)
  bigdft_pkg_string="BigDFT library 1.7.1.23 (abinit)"
  AC_SUBST(bigdft_pkg_string)

  dnl Define options associated to the package
  AC_ARG_ENABLE([BIGDFT],
    AC_HELP_STRING([--enable-bigdft],
      [BigDFT library 1.7.1.23 (abinit)]))
  ...
  if test "${enable_bigdft}" = "yes"; then
    ...
    if test "${with_bigdft_incs}" = "" -o "${with_bigdft_libs}" = ""; then
      ...
      if test -s "${abinit_tardir}/bigdft-1.7.1.23.tar.gz"; then
        AC_MSG_RESULT([yes])
        AX_CHECK_MD5SUM([${abinit_tardir}/bigdft-1.7.1.23.tar.gz],[0de95818065a9cc7f85675e1d7764a07])
        case "${abi_md5_ok}" in
          yes)
            afb_tarball="yes"
            AC_MSG_NOTICE([tarball MD5 check succeeded])
      ...
      dnl Get the package
      if test "${afb_ready}" = "no"; then
        for dl_url in "http://forge.abinit.org/fallbacks/bigdft-1.7.1.23.tar.gz"; do
          if test "${afb_tarball}" = "no"; then
            AC_MSG_NOTICE([downloading BIGDFT - this may take a while])
            rm -f "${abinit_tardir}/bigdft-1.7.1.23.tar.gz"
            ${afb_downloader} ${afb_dlopts}               "${abinit_tardir}/bigdft-1.7.1.23.tar.gz"               "${dl_url}"
            if test -s "${abinit_tardir}/bigdft-1.7.1.23.tar.gz"; then
              AX_CHECK_MD5SUM([${abinit_tardir}/bigdft-1.7.1.23.tar.gz],[0de95818065a9cc7f85675e1d7764a07])
              case "${abi_md5_ok}" in
                yes)
...
AC_DEFUN([ABI_FALLBACK_LIBXC],[
  ...
  dnl Define variables needed to build the package
  libxc_pkg_name="libxc-2.2.1"
  AC_SUBST(libxc_pkg_name)
  libxc_pkg_string="LibXC library 2.2.1 (upstream release)"
  AC_SUBST(libxc_pkg_string)

  dnl Define options associated to the package
  AC_ARG_ENABLE([LIBXC],
    AC_HELP_STRING([--enable-libxc],
      [LibXC library 2.2.1 (upstream release)]))
  ...
  if test "${enable_libxc}" = "yes"; then
    ...
    if test "${with_libxc_incs}" = "" -o "${with_libxc_libs}" = ""; then
      ...
      if test -s "${abinit_tardir}/libxc-2.2.1.tar.gz"; then
        AC_MSG_RESULT([yes])
        AX_CHECK_MD5SUM([${abinit_tardir}/libxc-2.2.1.tar.gz],[38dc3a067524baf4f8521d5bb1cd0b8f])
        case "${abi_md5_ok}" in
          yes)
            afb_tarball="yes"
            AC_MSG_NOTICE([tarball MD5 check succeeded])
      ...
      dnl Get the package
      if test "${afb_ready}" = "no"; then
        for dl_url in "http://www.tddft.org/programs/octopus/down.php?file=libxc/libxc-2.2.1.tar.gz" "http://forge.abinit.org/fallbacks/libxc-2.2.1.tar.gz"; do
          if test "${afb_tarball}" = "no"; then
            AC_MSG_NOTICE([downloading LIBXC - this may take a while])
            rm -f "${abinit_tardir}/libxc-2.2.1.tar.gz"
            ${afb_downloader} ${afb_dlopts}               "${abinit_tardir}/libxc-2.2.1.tar.gz"               "${dl_url}"
            if test -s "${abinit_tardir}/libxc-2.2.1.tar.gz"; then
              AX_CHECK_MD5SUM([${abinit_tardir}/libxc-2.2.1.tar.gz],[38dc3a067524baf4f8521d5bb1cd0b8f])
              case "${abi_md5_ok}" in
                yes)
