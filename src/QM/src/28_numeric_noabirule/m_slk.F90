...
!!
!! COPYRIGHT
!! Copyright (C) 2004-2016 ABINIT group (CS,GZ,FB,MG)
...
!! SOURCE
 ...
 type,public :: processor_scalapack
   ...
   integer :: comm
   ! MPI communicator MPI underlying the grid BLACS
   integer :: coords(2)

   type(grid_scalapack) :: grid   ! the grid to which the processor is associated
#ifdef HAVE_LINALG_ELPA
   integer :: comm_rows_elpa
   integer :: comm_cols_elpa
#endif
...
!! SOURCE
 ...
 type,public :: matrix_scalapack
   ...
   real(dp),allocatable  :: buffer_real(:,:)
     ! buffer of the local part of the matrix
...
#ifdef HAVE_LINALG_SCALAPACK
 public :: init_matrix_scalapack           ! Initialisation of a SCALAPACK matrix (each proc initialize its own part of the matrix)
 public :: destruction_matrix_scalapack    ! Destroys a matrix descriptor for ScaLAPACK.
 ...
 public :: idx_loc                         ! Local indices of a matrix coefficient with respect to its global indices, independently of the processor.
 public :: glob_loc                        ! Returns the global location of a matrix coefficient.
 public :: idx_glob                        ! Determination of the global indices of a term of the matrix with respect to its local indices.
 public :: loc_glob                        ! Determination of the global index from a local index (row or column) as a function of a given processor
 public :: matrix_from_global              ! Fills a SCALAPACK matrix with respect to a full matrix.
 public :: matrix_from_global_sym          ! Fills a SCALAPACK matrix with respect to a full matrix.
 public :: matrix_from_realmatrix          ! Fills a SCALAPACK matrix with respect to a full matrix.
 public :: matrix_from_complexmatrix       ! Fills a SCALAPACK matrix with respect to a full matrix.
 ...
 public :: slk_single_fview_read_mask        ! Returns an MPI datatype that can be used to read a scaLAPACK distributed matrix from
                                             !   a binary file using MPI-IO. The view is created using the user-defined mask function
 public :: slk_symmetrize                    ! Symmetrizes a square scaLAPACK-distributed matrix.
 public :: slk_single_fview_read             ! Returns an MPI datatype to read a scaLAPACK distributed matrix from a binary file using MPI-IO.
 public :: slk_single_fview_write            ! Returns an MPI datatype to write a scaLAPACK distributed matrix to a binary file using MPI-IO.
...
!except in the case where the mmyproc argument is not the local proc
 ...
 elpa_success=0
#ifdef HAVE_LINALG_ELPA
#ifdef HAVE_LINALG_ELPA_2015
 elpa_success=get_elpa_row_col_comms(comm, processor%coords(1), processor%coords(2),&
&  processor%comm_rows_elpa, processor%comm_cols_elpa)
#else
 call get_elpa_row_col_comms(comm, processor%coords(1), processor%coords(2),&
&  processor%comm_rows_elpa, processor%comm_cols_elpa)
#endif
 ABI_CHECK_MPI(elpa_success,"get_elpa_row_col_comms")
#endif
...
!!****f* m_slk/destruction_matrix_scalapack
!! NAME
...
!Likely gfortran will bomb out here.
 ...
 if (allocated(matrix%buffer_cplx)) then
   ABI_FREE(matrix%buffer_cplx)
 end if
 if (allocated(matrix%buffer_real)) then
   ABI_FREE(matrix%buffer_real)
 end if
 if (allocated(matrix%ipiv)) then
   ABI_FREE(matrix%ipiv)
 end if
...
#ifdef HAVE_LINALG_ELPA_2014
  if (istwf_k/=2) then
     elpa_success=solve_evp_complex(matrix%sizeb_global(1), matrix%sizeb_global(1), matrix%buffer_cplx, matrix%sizeb_local(1), &
&         eigen,results%buffer_cplx, results%sizeb_local(1), matrix%sizeb_blocs(1), &
&         processor%comm_rows_elpa, processor%comm_cols_elpa)
  else
     elpa_success=solve_evp_real(matrix%sizeb_global(1), matrix%sizeb_global(1), matrix%buffer_real, matrix%sizeb_local(1), &
&         eigen,results%buffer_real, results%sizeb_local(1), matrix%sizeb_blocs(1), &
&         processor%comm_rows_elpa, processor%comm_cols_elpa)
  end if
#else
  if (istwf_k/=2) then
    call solve_evp_complex(matrix%sizeb_global(1), matrix%sizeb_global(1), matrix%buffer_cplx, matrix%sizeb_local(1), &
&         eigen,results%buffer_cplx, results%sizeb_local(1), matrix%sizeb_blocs(1), &
&         processor%comm_rows_elpa, processor%comm_cols_elpa)
  else
    call solve_evp_real(matrix%sizeb_global(1), matrix%sizeb_global(1), matrix%buffer_real, matrix%sizeb_local(1), &
&         eigen,results%buffer_real, results%sizeb_local(1), matrix%sizeb_blocs(1), &
&         processor%comm_rows_elpa, processor%comm_cols_elpa)
  end if
  if (.not.elpa_success) then
    MSG_ERROR('Problem in ELPA use (solve_evp)!')
  end if
#endif
...
!  write(*,*) "    In compute_eigen_problem before PZHEEVX"
  ! Initialisation
  INFO   = 0
  ABSTOL = zero
  ORFAC  = -1.D+0
...
subroutine solve_gevp_complex(na,nev,na_rows,na_cols,nblk,a,b,ev,z,tmp1,tmp2, &
     my_prow,my_pcol,np_rows,np_cols,sc_desc,mpi_comm_rows,mpi_comm_cols)
...
!End of the abilint section
  ...
  elpa_success=.true.
#ifdef HAVE_LINALG_ELPA_2015
  call cholesky_complex(na, b, na_rows, nblk, mpi_comm_rows, mpi_comm_cols, .false., elpa_success)
#elif HAVE_LINALG_ELPA_2014
  call cholesky_complex(na, b, na_rows, nblk, mpi_comm_rows, mpi_comm_cols, elpa_success)
#else
  call cholesky_complex(na, b, na_rows, nblk, mpi_comm_rows, mpi_comm_cols)
#endif
  if (.not.elpa_success) then
    MSG_ERROR('Problem in ELPA use (choleski_complex)!')
  end if
#ifdef HAVE_LINALG_ELPA_2015
  call invert_trm_complex(na, b, na_rows, nblk, mpi_comm_rows, mpi_comm_cols, .false., elpa_success)
#elif HAVE_LINALG_ELPA_2014
  call invert_trm_complex(na, b, na_rows, nblk, mpi_comm_rows, mpi_comm_cols, elpa_success)
#else
  call invert_trm_complex(na, b, na_rows, nblk, mpi_comm_rows, mpi_comm_cols)
#endif
  if (.not.elpa_success) then
    MSG_ERROR('Problem in ELPA use (invert_trm_complex)!')
  end if
...
#ifdef HAVE_LINALG_ELPA_2014
  elpa_success=solve_evp_complex(na, nev, a, na_rows, ev, tmp1, na_rows, nblk, &
       mpi_comm_rows, mpi_comm_cols)
  if (.not.elpa_success) then
    MSG_ERROR('Problem in ELPA use (solve_evp_complex)!')
  end if
#else
...
!End of the abilint section
  ...
  elpa_success=.true.
#ifdef HAVE_LINALG_ELPA_2015
  call cholesky_real(na, b, na_rows, nblk, mpi_comm_rows, mpi_comm_cols, .false., elpa_success)
#elif HAVE_LINALG_ELPA_2014
  call cholesky_real(na, b, na_rows, nblk, mpi_comm_rows, mpi_comm_cols, elpa_success)
#else
  call cholesky_real(na, b, na_rows, nblk, mpi_comm_rows, mpi_comm_cols)
#endif
  if (.not.elpa_success) then
    MSG_ERROR('Problem in ELPA use (choleski_real)!')
  end if
#ifdef HAVE_LINALG_ELPA_2015
  call invert_trm_real(na, b, na_rows, nblk, mpi_comm_rows, mpi_comm_cols, .false., elpa_success)
#elif HAVE_LINALG_ELPA_2014
  call invert_trm_real(na, b, na_rows, nblk, mpi_comm_rows, mpi_comm_cols, elpa_success)
#else
  call invert_trm_real(na, b, na_rows, nblk, mpi_comm_rows, mpi_comm_cols)
#endif
  if (.not.elpa_success) then
    MSG_ERROR('Problem in ELPA use (invert_trm_real)!')
  end if
  ! 2. Calculate U**-T * A * U**-1
  ! 2a. tmp1 = U**-T * A
...
&       nblk, mpi_comm_rows, mpi_comm_cols, a, na_rows)
  ...
  !    Eigenvectors go to tmp1
#ifdef HAVE_LINALG_ELPA_2014
  elpa_success=solve_evp_real(na, nev, a, na_rows, ev, tmp1, na_rows, nblk, &
&       mpi_comm_rows, mpi_comm_cols)
  if (.not.elpa_success) then
    MSG_ERROR('Problem in ELPA use (solve_evp_real)!')
  end if
#else
...
&          matrix1%descript%tab,processor%comm_rows_elpa, processor%comm_cols_elpa)
  else
...
&          matrix1%descript%tab,processor%comm_rows_elpa, processor%comm_cols_elpa)
  ...
  call destruction_matrix_scalapack(tmp1)
  call destruction_matrix_scalapack(tmp2)
#else
!Arguments ------------------------------------
  type(processor_scalapack),intent(in)       :: processor
  type(matrix_scalapack),intent(in)          :: matrix1,matrix2
...
&    MPI_SUM,comm,ierr)
 ...
 ! ====================================
 call destruction_matrix_scalapack(sca_matrix1)
 call destruction_matrix_scalapack(sca_matrix2)
...
&    MPI_SUM,comm,ierr)
 ...
 call destruction_matrix_scalapack(sca_matrix1)
 call destruction_matrix_scalapack(sca_matrix2)
 call destruction_matrix_scalapack(sca_matrix3)
...
!One and only one buffer should be allocated.
 ...
 case ("A","a") ! Full global matrix has been read, nothing to do.
   RETURN

 case ("U","u") ! Only the upper triangle of the global matrix was read.
...
!        if (iglob==jglob) Slk_mat%buffer_cplx(iloc,jloc) =  real(Slk_mat%buffer_cplx(iloc,jloc))
 ...
 case ("L","l") ! Only the lower triangle of the global matrix was read.
 ...
 case ("C")
   ...
   do col_loc=1,Slk_mat%sizeb_local(2)
     do row_loc=1,Slk_mat%sizeb_local(1)
       call idx_glob(Slk_mat,row_loc,col_loc,row_glob,col_glob)
       !
