...
!!
!! PARENTS
!!      dfpt_cgwf,dfpt_nstpaw,dfpt_nstwf,dfpt_wfkfermi,m_d2dk,m_phgamma
!!
!! CHILDREN
...
!Local variables-------------------------------
!scalars
 integer :: dimffnl1,dimffnlk,ider,idir0,istr,ntypat
 logical :: qne0
!arrays
 integer :: idirs(2)
...
! *************************************************************************
 ...
 if((ipert==natom+10.or.ipert==natom+11) .and. .not.present(ddkinpw)) then
   MSG_BUG("ddkinpw is not optional for ipert = natom+10 or 11")
 end if
 if(ipert==natom+11 .and. ((.not.present(dkinpw2)).or.(.not.present(rf_hamk_dir2)))) then
   MSG_BUG("dkinpw2 and rf_hamk_dir2 are not optional for ipert = natom +11")
 end if
...
!For d2dk perturbation, cprj coeffs are computed on-the-fly in nonlop
...
!  Compute the derivative of the kinetic operator vs k in kinpw, second and first orders
   if(ipert==natom+10) then
     ...
     call mkkin(dtset%ecut,dtset%ecutsm,dtset%effmass,gmet,kg_k,dkinpw,kpoint,npw_k,idirs(1),0) !  1st derivative, idir1
     call mkkin(dtset%ecut,dtset%ecutsm,dtset%effmass,gmet,kg_k,dkinpw2,kpoint,npw_k,idirs(2),0) ! 1st derivative, idir2
     call mkkin(dtset%ecut,dtset%ecutsm,dtset%effmass,gmet,kg_k,ddkinpw,kpoint,npw_k,idirs(1),idirs(2)) ! 2nd derivative
   end if
 end if
