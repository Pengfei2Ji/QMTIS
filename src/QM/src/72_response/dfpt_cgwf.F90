...
!!
!! COPYRIGHT
!! Copyright (C) 1999-2016 ABINIT group (XG,DRH,XW,FJ,MT,LB)
...
subroutine dfpt_cgwf(band,berryopt,cgq,cwavef,cwave0,cwaveprj,cwaveprj0,d2dk,dcwavef,&
...
!  ===== Check Pc.dcwavef=0 (for 2nd order only)
   if(ipert==natom+10.or.ipert==natom+11) then
     work(:,:)=d2dk%dcwavef(:,1+shift_band:npw1*nspinor+shift_band)
     call projbd(cgq,work,-1,icgq,igscq,istwf_k,mcgq,mgscq,nband,npw1,nspinor,&
&     gscq,scprod,0,tim_projbd,useoverlap,me_g0,comm_fft)
     call sqnorm_g(dotr,istwf_k,npw1*nspinor,work,me_g0,comm_fft)
...
!  Otherwise, the work is already done in d2dk_init (called in dfpt_vtowfk.F90)
...
&       mpi_enreg,natom,nband,npw1,nspinor,0,gs1c)
 ...
 else ! 2nd order case (wrt k perturbation)
   ! Copy RHS_Stern(:,:) of the given band in gh1c
   gh1c(:,:)=d2dk%RHS_Stern(:,1+shift_band:npw1*nspinor+shift_band)
 end if
...
&   cgq,scprod,0,tim_projbd,useoverlap,me_g0,comm_fft)
 ...
 if(ipert/=natom+10.and.ipert/=natom+11) call cg_zcopy(npw1*nspinor,gh1c,gh1c_n)
...
&   dummy,scprod,0,tim_projbd,useoverlap,me_g0,comm_fft)
 ...
 if(ipert/=natom+10.and.ipert/=natom+11) then
 ! For ipert=natom+10 or natom+11, this is done in d2dk_init
...
!$OMP PARALLEL DO
 ...
 else ! In 2nd order case, dcwavef/=0 even in NC, and it is already computed in d2dk_init (called in dfpt_vtowfk.F90)
   do ipw=1,npw1*nspinor
     cwavef(:,ipw)=cwavef(:,ipw)+d2dk%dcwavef(:,ipw+shift_band)
   end do
 end if
...
!    between E(2) and the 2DTE
     ...
     dedt=-two*two*dedt
     if((prtvol==-level.or.prtvol==-19).and.dedt-tol14>0) then
...
&           mpi_enreg,optlocal,optnl,opt_gvnl1,rf_hamkq,sij_opt,tim_getgh1c,usevnl)
         ...
         dotr=prod1+half*d2dk%amn(1,iband+(band-1)*nband)
         doti=prod2+half*d2dk%amn(2,iband+(band-1)*nband)
       else
...
!    -Add delta_Psi^(1)
     ...
     if(ipert==natom+10.or.ipert==natom+11) cwwork=cwwork+d2dk%dcwavef(:,1+shift_band:npw1*nspinor+shift_band)
!    -Compare to Psi^(1)
