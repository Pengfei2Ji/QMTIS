...
!Local variables-------------------------------
 ...
 integer :: req_cplex_dij,rfasr,rfddk,rfelfd,rfphon,rfstrs,rfuser,rf2_dkdk
...
!Respfn input variables
 ...
 rfuser=dtset%rfuser ; rf2_dkdk=dtset%rf2_dkdk
 has_allddk = .False.
...
!Initialize the list of perturbations rfpert
 ...
 if(rf2_dkdk==1.or.rf2_dkdk==3)rfpert(natom+10)=1
 if(rf2_dkdk==2.or.rf2_dkdk==3)rfpert(natom+11)=1
...
&   '  q has been set exactly to (0 0 0)'
   ...
   if(rfelfd/=0 .or. rfstrs/=0 .or. rfddk /= 0  .or. rf2_dkdk /= 0) then
!    Temporarily, ...
...
&     'while rfelfd=',rfelfd,', rfddk=',rfddk,', rf2_dkdk=',rf2_dkdk,' and rfstrs=',rfstrs,'.',ch10,&
...
!Were 2DTE computed ?
 if(rfphon==0 .and. (rf2_dkdk/=0 .or. rfddk/=0 .or. rfelfd==2) .and. rfstrs==0 .and. rfuser==0)then
...
!Compute the other terms for AHC dynamic and AHC full
 if (.not.(rfphon==0 .and. (rf2_dkdk/=0 .or. rfddk/=0 .or. rfelfd==2) .and. rfstrs==0 .and. rfuser==0)) then
