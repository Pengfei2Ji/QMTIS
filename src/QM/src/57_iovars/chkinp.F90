...
!!
!! COPYRIGHT
!! Copyright (C) 1998-2016 ABINIT group (DCA, XG, GMR, MKV, DRH, MVer)
...
!  Copy or initialize locally a few input dataset values
   ...
   response=0
   if(dt%rfelfd/=0.or.dt%rfphon/=0.or.dt%rfstrs/=0.or.dt%rfddk/=0.or.dt%rfuser/=0.or.dt%rf2_dkdk/=0)response=1
     ...
     do isym=1,dt%nsym
       do ii=1,3
         ...
         if(abs(delta-nint(delta))>tol6)then
           ...
           if(abs(delta-nint(delta))>tol6)then
             write(message, '(8a,i4,2a,9i3,2a,3es16.6,4a)' ) ch10,&
&             ' chkinp: WARNING -',ch10,&
...
!  dmftbandi, dmftbandf
   if (dt%usedmft>0) then
     ...
     if(dt%dmftcheck/=-1) then
       ...
       if(dt%getwfk==0.and.dt%irdwfk==0.and.dt%irdden==0.and.dt%getden==0) then
   ...
   do ikpt=1,nkpt
     do mu=1,3
       if ( abs(dt%kpt(mu,ikpt))> dt%kptnrm*1.0000001_dp ) then
         write(message, '(a,i5,a,a,a,a,3es22.14,a,a,a,a)' )&
       ...
       do itypat=1,dt%ntypalch
         ...
         if(abs(sumalch-one)>tol10)then
           if(dt%npspalch<=6)then
             write(message, '(2a,6es12.4)' )ch10,' chkinp : mixalch(:,itypat,iimage)=',dt%mixalch_orig(:,itypat,iimage)
           end if
           call wrtout(iout,message,'COLL')
           call wrtout(std_out,  message,'COLL')
           write(message, '(a,i4,2a,i4,2a,f8.2,4a)' )&
...
!  When nsppol==2 and spinmagntarget is the default value (-99.99d0), occopt cannot be 1.
   if(nsppol==2.and.dt%occopt==1.and.abs(dt%spinmagntarget+99.99d0)<tol8)then
     if(natom/=1 .or. abs(dt%znucl(dt%typat(1))-one)>tol8)then
       write(message,'(a,i3,2a,i3,4a,f7.2,7a)' )&
...
&   [RUNL_GSTATE,RUNL_RESPFN,RUNL_SCREENING,RUNL_SIGMA,RUNL_NONLINEAR,RUNL_BSE, RUNL_GWLS, RUNL_WFK,RUNL_EPH],iout)
   if (response==1.and.dt%optdriver/=1) then
     write(message,'(a,i3,4a,5(i3,a),a,i3,4a)' )&
&     'The input variable optdriver=',dt%optdriver,ch10,&
&     'This is in conflict with the values of the other input variables,',ch10,&
&     'rfphon=',dt%rfphon,'  rfddk=',dt%rfddk,'  rf2_dkdk=',dt%rf2_dkdk,'  rfelfd=',dt%rfelfd,&
&     'rfstrs=',dt%rfstrs,ch10,'  rfuser=',dt%rfuser,ch10,&
&     'Action : check the values of optdriver, rfphon, rfddk, rf2_dkdk, rfelfd, rfstrs',ch10,&
...
!  Non-linear response calculations
   ...
   if(.not.allow)then
     allow=(dt%ixc < 0).and.(libxc_functionals_isgga().or.libxc_functionals_ismgga())
   end if
   if(allow)then
...
&     'rfphon=',dt%rfphon,'  rfddk=',dt%rfddk,'  rf2_dkdk=',dt%rf2_dkdk,'  rfelfd=',dt%rfelfd,&
&     'rfstrs=',dt%rfstrs,ch10,'  rfuser=',dt%rfuser,ch10,&
&     'Action: check the values of paral_rf, rfphon, rfddk, rf2_dkdk, rfelfd, rfstrs',ch10,&
&     '        and rfuser in your input file.'
     MSG_ERROR_NOSTOP(message, ierr)
   end if
...
!  pawfatbnd
   ...
   if(usepaw/=1.and.dt%pawfatbnd>1) then
     message = 'pawfatbnd without PAW is not possible'
     MSG_ERROR_NOSTOP(message,ierr)
   end if
...
&       '=== Coarse Grid is used in recursion ==========================================='
       call wrtout(ab_out,message,'COLL')
       write(message, '(a,i3,a,a,i3,a,i3,a,i3)' ) 'grid ratio =',dt%recgratio,&
...
!       Only up to chemical species 96
       do itypat=1,dt%ntypat
         if (dt%znucl(itypat)<0.or.dt%znucl(itypat)>96) then
           write(message,'(4a,i2,1a,f5.1,a)') ch10,&
&           ' chkinp : ERROR -',ch10,&
...
!  wfoptalg==0,1,4,10 or 14 if PAW
   ...
   if (dt%npfft > 1 .and..not. fftalg_has_mpi(fftalg)) then
     write(message,"(a,i0,a)")"fftalg: ",fftalg," cannot be used in MPI-FFT mode (npfft > 1)"
     MSG_ERROR_NOSTOP(message,ierr)
   end if
   ...
   if(dt%wfoptalg == 1) then
     if(dt%nspinor > 1) then
       message='Nspinor > 1 not yet compatible with wfoptalg 1'
       MSG_ERROR_NOSTOP(message, ierr)
     end if
     !! TODO obsolete?
     if(dt%usefock > 0) then
       message='Fock not yet compatible with wfoptalg 1 (use Fock-level parallelism)'
       MSG_ERROR_NOSTOP(message, ierr)
     end if
     if(maxval(abs(dt%istwfk(1:nkpt))) > 2) then
       message='Istwfk > 2 not compatible with wfoptalg 1'
       MSG_ERROR_NOSTOP(message, ierr)
     end if
     if(dt%ecutsm > 0) then
       message='Ecutsm > 0 not yet compatible with wfoptalg 1'
       MSG_ERROR_NOSTOP(message, ierr)
     end if
...
&           abs(frac(3,ia)-frac(3,ib))<1.0d-6         ) then
             if(iimage>1)then
               write(message,'(2a,i5)') ch10,' The following was observed for image=',iimage
               call wrtout(iout,message,'COLL')
               call wrtout(std_out,message,'COLL')
             end if
             write(message, '(a,i4,a,i4,a,a,a,a,a,a)' )&
...
!  Check that znucl and znuclpsp agree
   do ipsp=1,npsp
     if (abs(dt%znucl(ipsp)-pspheads(ipsp)%znuclpsp)> tol12 ) then
       write(message, '(a,i0,a,es12.4,a,a,es12.4,2a)' )&
...
!If there was a problem, then stop.
 call xmpi_sum(ierr_dtset,xmpi_world,mpierr)
