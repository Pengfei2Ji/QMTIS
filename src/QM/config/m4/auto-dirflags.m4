...
# Generated by make-macros-dirflags on 2016/05/22 07:08:35 +0000
...
AC_DEFUN([ABI_OPTFLAGS_DIRS],[
  ...
  dnl 01_triqs_ext library
  if test "${FCFLAGS}" = ""; then
    if test "${fcflags_opt_01_triqs_ext}" = "" -o "${enable_optim}" = "no"; then
      fcflags_opt_01_triqs_ext="${fcflags_opt_default}"
    else
      AC_MSG_NOTICE([optimization for 01_triqs_ext is ${fcflags_opt_01_triqs_ext}])
      echo "fcflags_opt_01_triqs_ext='${fcflags_opt_01_triqs_ext}'" >>config.optim
      fcflags_opt_dirlist="${fcflags_opt_dirlist} 01_triqs_ext"
    fi
  else
    if test "${fcflags_opt_01_triqs_ext}" != ""; then
      fcflags_opt_01_triqs_ext=""
      AC_MSG_NOTICE([fcflags_opt_01_triqs_ext overriden by FCFLAGS])
    fi
  fi
  AC_SUBST(fcflags_opt_01_triqs_ext)
  ...
  dnl 20_datashare library
  if test "${FCFLAGS}" = ""; then
    if test "${fcflags_opt_20_datashare}" = "" -o "${enable_optim}" = "no"; then
      fcflags_opt_20_datashare="${fcflags_opt_default}"
    else
      AC_MSG_NOTICE([optimization for 20_datashare is ${fcflags_opt_20_datashare}])
      echo "fcflags_opt_20_datashare='${fcflags_opt_20_datashare}'" >>config.optim
      fcflags_opt_dirlist="${fcflags_opt_dirlist} 20_datashare"
    fi
  else
    if test "${fcflags_opt_20_datashare}" != ""; then
      fcflags_opt_20_datashare=""
      AC_MSG_NOTICE([fcflags_opt_20_datashare overriden by FCFLAGS])
    fi
  fi
  AC_SUBST(fcflags_opt_20_datashare)
