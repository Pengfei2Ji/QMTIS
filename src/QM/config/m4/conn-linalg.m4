...
# _ABI_LINALG_CHECK_ELPA_2014(ELPA,EXTRA_LIBS)
# --------------------------------------------
#
...
# _ABI_LINALG_CHECK_ELPA_2015(ELPA,EXTRA_LIBS)
# --------------------------------------------
#
...
AC_DEFUN([_ABI_LINALG_CHECK_ELPA_2015],[
  ...
  AC_COMPILE_IFELSE([AC_LANG_PROGRAM([],
    [
    use elpa1
    integer,parameter :: na_r=1,na_c=1
    integer :: nblk=1,comm=1,comm_r=1,comm_c=1
    logical :: debug=.false.,success
    real*8 :: a(na_r,na_c)
    call cholesky_real(na_r,a,na_r,nblk,comm_r,comm_c,debug,success)
    success=get_elpa_row_col_comms(comm,na_r,na_c,comm_r,comm_c)
