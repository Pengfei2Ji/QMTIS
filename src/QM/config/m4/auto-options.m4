...
# Generated by make-macros-options on 2016/05/22 07:08:34 +0000
...
AC_DEFUN([ABI_OPTIONS_DEFINE],[
  ...
  AC_ARG_ENABLE(mpi-io-default,
    AC_HELP_STRING([--enable-mpi-io-default],
      [Use MPI-IO as default IO library (change the default value of accesswff) (EXPERIMENTAL) (default: no)]))
  AC_SUBST(enable_mpi_io_default)

  AC_ARG_ENABLE(netcdf-default,
    AC_HELP_STRING([--enable-netcdf-default],
      [Use NetCDF as default IO library (change the default value of accesswff) (default: no)]))
  ...
  AC_ARG_ENABLE(triqs,
    AC_HELP_STRING([--enable-triqs],
      [Activate internal support for TRIQS (default: no)]))
  AC_SUBST(enable_triqs)
...
AC_DEFUN([ABI_OPTIONS_RECALL],[
  ...
  if test "${cmd_enable_triqs}" != ""; then
    enable_triqs="${cmd_enable_triqs}"
  fi
...
AC_DEFUN([ABI_OPTIONS_CHANGED],[
  ...
  dnl New --enable-avx-safe-mode option
  AC_MSG_NOTICE([ * new option --enable-avx-safe-mode is available])
  ...
  AC_MSG_NOTICE([ * --enable-clib removed])
  ...
  dnl New --enable-mpi-io-default option
  AC_MSG_NOTICE([ * new option --enable-mpi-io-default is available])
  ...
  dnl New --enable-netcdf-default option
  AC_MSG_NOTICE([ * new option --enable-netcdf-default is available])
  ...
  AC_MSG_NOTICE([ * --with-libpspio-libs removed])
...
AC_DEFUN([ABI_OPTIONS_PARSE],[
  ...
  dnl Parse enable_triqs
  if test "${enable_triqs}" != ""; then
    for v in `echo "${enable_triqs}" | sed -e 's/+/ /g'`; do
      opt_ok="no"
      for r in no yes; do
        if test "${v}" = "${r}";then
          opt_ok="yes"
    ...
    if test "${opt_ok}" = "no"; then
      AC_MSG_WARN([enable_triqs = (no|yes)])
      AC_MSG_ERROR([invalid option: enable_triqs = ${v}])
    fi
  fi
 ...
 if test "${with_psml_libs}" != ""; then
    ...
    if test "${opt_ok}" = "no"; then
      AC_MSG_ERROR([invalid library statement in with_psml_libs: ${v}])
    fi
  fi
 ...
 if test "${with_timer_libs}" != ""; then
  ...
  if test "${with_trio_flavor}" != ""; then
    for v in `echo "${with_trio_flavor}" | sed -e 's/+/ /g'`; do
      opt_ok="no"
      for r in etsf_io etsf_io-fallback psml netcdf netcdf-fallback yaml none; do
        if test "${v}" = "${r}";then
          opt_ok="yes"
          break
        fi
    ...
    if test "${opt_ok}" = "no"; then
      AC_MSG_WARN([with_trio_flavor = (etsf_io|etsf_io-fallback|psml|netcdf|netcdf-fallback|yaml|none)])
 ...
 if test "${with_triqs_libs}" != ""; then
    ...
    if test "${opt_ok}" = "no"; then
      AC_MSG_ERROR([invalid library statement in with_triqs_libs: ${v}])
    fi
  fi
...
AC_DEFUN([ABI_OPTIONS_TRIGGERS],[
  ...
  AC_MSG_CHECKING([whether to use MPI-IO as default IO library (change the default value of accesswff) (EXPERIMENTAL)])
  ...
  if test "${enable_mpi_io_default}" = "yes"; then
    ...
    AC_DEFINE([HAVE_MPI_IO_DEFAULT],1,
      [Define to 1 if you want to use MPI-IO as default IO library (change the default value of accesswff) (EXPERIMENTAL).])
  fi

  dnl Triggers for enable_netcdf_default
  AC_MSG_CHECKING([whether to use NetCDF as default IO library (change the default value of accesswff)])
  ...
  if test "${enable_netcdf_default}" = "yes"; then
    ...
    AC_DEFINE([HAVE_NETCDF_DEFAULT],1,
      [Define to 1 if you want to use NetCDF as default IO library (change the default value of accesswff).])
  fi
  ...
  dnl Triggers for enable_triqs
  AC_MSG_CHECKING([whether to activate internal support for TRIQS])
  AC_MSG_RESULT([${enable_triqs}])

  if test "${enable_triqs}" = "yes"; then
    AC_MSG_NOTICE([defining the 'HAVE_TRIQS' preprocessing macro])
  ...
  if test "${enable_wannier90_v1}" = "yes"; then
    AC_MSG_NOTICE([defining the 'HAVE_DFT_WANNIER90_V1' preprocessing macro])
